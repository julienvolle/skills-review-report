...
class InterviewService implements FlashBagAwareInterface, SecurityAwareInterface
{
     ...
     * @param FrameworkService       $frameworkService
    ...
    public function import(Interview $interview, bool $force = false): Interview
    {
        try {
            ...
            if (!$currentInterview = $this->search($interview->getGuid())) {
                $this->save($interview);
                $this->addFlash('success', $this->translator->trans('flash.interview.imported', [], 'alertes'));
        ...
        if (!$this->equals($currentInterview, $interview)) {
            if ($force) {
                try {
                    ...
                    $this->addFlash('success', $this->translator->trans('flash.interview.updated', [], 'alertes'));
        ...
        $this->addFlash('info', $this->translator->trans('flash.interview.up_to_date', [], 'alertes'));
     ...
     *
     * @param Interview $interviewToDelete
     * @param Interview $interviewToCreate
     *
    ...
    {
        return hash('sha256', $this->serializer->serialize($interview, SerializerConstant::FORMAT_EXPORT, [
            'groups' => [
